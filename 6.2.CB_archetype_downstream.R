#!/usr/bin/env Rscript
# 6.2.archetype_downstream_CB.R
# Cross-sample downstream analysis of archetypal analysis results, for the
# CB-only run (6.1.archetypal_analysis_tumour_CB.ipynb — archetypes fit on
# category containing "CB" cells rather than the full tumour population).
# Reads per-sample 6.1.CB_archetype/{sample}/ CSVs + native tumour_srt.qs2, then:
...
#     (subset to the CB cells the weights were fit on), scores each archetype's
#     top DE genes (AddModuleScore), and renders cross-sample spatial maps
#     (ImageDimPlot/ImageFeaturePlot); exports metadata
# Output: VisHD/6.2.archetype_downstream_tumour_CB/
...
# Local override of do.spanorm() for this script's per-sample CB-only subsets:
# identical to do.spanorm() except the final Leiden clustering step falls back
# to algorithm = 1 (Louvain) if Leiden errors out. Degenerate SpaNorm size
# factors / SNN graphs can crash Seurat's group.singletons() inside
# algorithm = 4 (see functions.R do.spanorm — unmodified there since it's
# shared by ~20 other pipeline scripts that run on much larger objects without
# hitting this edge case).
do.spanorm_cb <- function(srt, outdir = ".") {
  ...
  spatial_coord <- tryCatch(
    ...
    error = function(e) {
      if (all(c("x_centroid", "y_centroid") %in% colnames(srt@meta.data))) {
        ...
        stop("do.spanorm_cb: no image and no x_centroid/y_centroid in meta.data")
      }
    }
  )
  ...
  # scran::quickCluster() inside SpaNorm's fitSpaNorm() errors ("fewer cells
  # than the minimum cluster size") when a CB-subset is too small for its
  # default pooling. Rather than tuning that internal, unexposed threshold,
  # fall back to returning srt with its raw Spatial counts untouched — the
  # caller already handles a missing "SpaNorm" assay by scoring off
  # log-normalized "Spatial" data instead (see score_assay below).
  srt_normed <- tryCatch({
    ...
    tryCatch(
      ...
      error = function(e) {
        message("  do.spanorm_cb: Leiden clustering failed (", conditionMessage(e),
                ") — falling back to algorithm = 1 (Louvain)")
        FindClusters(srt, resolution = 1, algorithm = 1)
      }
    )
  }, error = function(e) {
    message("  do.spanorm_cb: SpaNorm failed (", conditionMessage(e),
            ") — likely too few cells in this CB subset; skipping SpaNorm, ",
            "keeping raw Spatial counts")
    NULL
  })

  if (!is.null(srt_normed)) return(srt_normed)

  # srt may carry a "SpaNorm" assay inherited from the full tumour population
  # (fit before this CB subset was taken) — drop it so the caller's
  # score_assay check (do.spanorm_cb: no SpaNorm -> Spatial) doesn't pick up
  # normalisation fit on a different, much larger cell population.
  assay_names <- tryCatch(as.character(Assays(srt)), error = function(e) character(0))
  if ("SpaNorm" %in% assay_names) srt[["SpaNorm"]] <- NULL
...
base_dir  <- "~/VisHD"
outdir    <- file.path(base_dir, "6.2.archetype_downstream_tumour_CB")
...
# Per-sample CB-subset Seurat objects (re-SpaNormed, with archetype metadata)
cbdir <- file.path(outdir, "CB_srt")
...
for (i in seq_along(samples)) {
  s          <- samples[i]
  result_dir <- file.path(base_dir, "6.1.CB_archetype", s)
  ...
  # The CB notebook subsets tumour.h5ad to category containing "CB" *before*
  # fitting archetypes, so AA_cell_weights row names are 0-based Python
  # positions into the FULL tumour object (not barcodes). Subset to the same
  # CB cells directly here, then check that the position-derived index agrees
  # before trusting it.
  # grepl() (unlike `==`) drops names(), so which(grepl(...)) comes back
  # unnamed — index the names vector directly instead.
  cb_pos   <- names(srt_full$category)[grepl("CB", srt_full$category)]
  w        <- read.csv(weights_f, row.names = 1, check.names = FALSE)
  cell_pos <- rownames(w)

  if (anyNA(cell_pos) || length(cb_pos) != nrow(w) || !identical(cb_pos, cell_pos)) {
    message("  ", s, ": CB-subset cells (", length(cb_pos), ") don't match weight row positions (",
  ...
  # SpaNorm on just the CB-subset cells so normalisation reflects this population.
  srt <- do.spanorm_cb(srt)
  ...
  md$slide   <- s
  meta_list[[s]] <- md
  qs_save(srt, file.path(cbdir, sprintf("%s_CB_srt.qs2", s)))
  rm(srt); gc()

  }, error = function(e) {
    message("  Skipping ", s, " — error during per-cell archetype processing: ",
            conditionMessage(e))
  })
}
