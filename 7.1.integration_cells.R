...
if (file.exists(pearson_path)) {
  ...
  srt_list <- lapply(seq_along(paths), function(i) {
    ...
    counts   <- GetAssayData(srt_full, assay = "RNA", layer = "counts")
    ...
    srt_mini <- CreateSeuratObject(counts = counts, meta.data = meta)
    rm(srt_full); gc()
    srt_mini
  })
  ...
  counts_mat <- GetAssayData(srt, assay = "RNA", layer = "counts")
...
# ── QC plots ───────────────────────────────────────────────────────────────
qc_vars <- c("nCount_RNA", "nFeature_RNA")
...
if (!file.exists(h5ad_path)) {
  ...
  counts_mat <- t(GetAssayData(srt, assay = "RNA", layer = "counts"))
  data_mat   <- t(GetAssayData(srt, assay = "RNA", layer = "data"))
