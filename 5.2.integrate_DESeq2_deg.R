...
# Drop low-cell pseudobulks. The result is the "unpaired universe" used by the
# unpaired analyses below — it keeps DT-only samples (e.g. LUT-245-09/10) that
# the paired filter would otherwise discard.
...
# Paired subset: keep only sample_subclones with BOTH DT and CB present (>=min_cells)
...
cat(nrow(pb_meta), " paired pseudobulks (", length(paired), " pairs); ",
    nrow(pb_meta_all), " total pseudobulks for unpaired analyses\n", sep = "")
...
saveRDS(dds,    file.path(paired_dir, "deseq2_dds.Rds"))
saveRDS(res_df, file.path(paired_dir, "deseq2_res_DT_vs_CB.Rds"))
write.csv(res_df, file.path(paired_dir, "deseq2_res_DT_vs_CB.csv"), row.names = FALSE)
...
if (nrow(sig_df) > 0) {
  ...
  saveRDS(enrich_list, file.path(paired_dir, "enrich_DT_vs_CB.Rds"))
...
write.csv(summary_df, file.path(paired_dir, "DEG_summary.csv"), row.names = FALSE)
print(summary_df)
...
if (length(unique(pb_meta_dt$cohort)) < 2 || ncol(pb_counts_dt) < 4) {
  ...
  saveRDS(dds_co,     file.path(paired_dir, "deseq2_dds_cohortB_vs_A_DT.Rds"))
  saveRDS(res_co_df,  file.path(paired_dir, "deseq2_res_cohortB_vs_A_DT.Rds"))
  write.csv(res_co_df, file.path(paired_dir, "deseq2_res_cohortB_vs_A_DT.csv"),
            row.names = FALSE)
  ...
  if (nrow(sig_co_df) > 0) {
    ...
    saveRDS(enrich_co, file.path(paired_dir, "enrich_cohortB_vs_A_DT.Rds"))
...
# Used for the two unpaired comparisons below, which draw from pb_meta_all /
# pb_counts_all (every pseudobulk with >=min_cells, no pairing requirement) so
# DT-only samples like LUT-245-09/10 are included.
...
run_unpaired_block <- function(counts, meta, group_var, contrast,
  ...
  # Lowly-expressed gene filter (same rule as the paired analysis)
...
# 11 ── Unpaired DT vs CB across ALL samples (design ~category_bin) ───────────
ud <- file.path(outdir, "unpaired_DT_vs_CB")
run_unpaired_block(
  ...
  title_tag  = "DT vs CB — unpaired (all samples)")

# 12 ── Unpaired cohort B vs A among ALL DT pseudobulks (design ~cohort) ──────
# Reuses cohortA / cohortB defined in section 9.
uc        <- file.path(outdir, "unpaired_cohort_B_vs_A_DT")
meta_co_u <- pb_meta_all
meta_co_u$cohort <- ifelse(meta_co_u$sample_id %in% cohortA, "A",
                    ifelse(meta_co_u$sample_id %in% cohortB, "B", NA_character_))
meta_co_u <- meta_co_u[meta_co_u$category_bin == "DT" & !is.na(meta_co_u$cohort), , drop = FALSE]
run_unpaired_block(
  counts     = pb_counts_all[, rownames(meta_co_u), drop = FALSE],
  meta       = meta_co_u,
  group_var  = "cohort",
  contrast   = c("cohort", "B", "A"),
  out_dir    = uc,
  group_cols = c(A = "steelblue", B = "firebrick"),
